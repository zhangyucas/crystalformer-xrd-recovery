#!/usr/bin/env python
"""Run the controlled old-cosine versus peak-reward PPO training matrix."""
...
def completed_run(folder: Path, epochs: int) -> tuple[bool, str, Path | None]:
    ...
    expected_checkpoints = [10000 + epoch for epoch in range(5, epochs + 1, 5)]
    checkpoints = [run_dir / f"epoch_{epoch:06d}.pkl" for epoch in expected_checkpoints]
    if len(rows) != epochs:
...
def main() -> None:
    ...
    if args.max_hours <= 0 or args.epochs <= 0 or args.epochs % 5:
        parser.error("max-hours must be positive and epochs must be a multiple of 5")
    ...
    target_root = repo / "issue68_xrd_recovery/experiments/benchmark_20260731/opxrd_subset"
    ...
    for case_index, (formula, case_dir) in enumerate(CASES, start=1):
        target = target_root / case_dir / "target.csv"
        for seed_offset in SEED_OFFSETS:
            seed = 9300 + case_index * 10 + seed_offset
            for method in METHODS:
                ...
                if remaining < 600:
                    ...
                    manifest["stop_reason"] = "less_than_10_minutes_remaining"
                    write_manifest(manifest_path, manifest)
                    return
                ...
                train_command = [
                    ...
                    "--reward", "xrd", "--xrd_score", method,
                    "--formula", formula, "--xrd_target", str(target),
                    "--restore_path", str(checkpoint), "--folder", str(folder),
                    "--epochs", str(args.epochs), "--checkpoint_interval", "5",
                    "--ppo_epochs", "1", "--batchsize", "8",
                    "--sampling_batchsize", "1", "--ppo_microbatch_size", "2",
                    "--sample_multiplier", "1", "--max_sampling_attempts", "256",
                    "--composition_max_atoms", "40", "--composition_size_bias", "0.5",
                    "--beta", "0.1", "--gamma", "0.1", "--alpha", "0",
                    "--lr", "5e-7", "--reset_optimizer", "--seed", str(seed),
                    "--K", "0", "--num_io_process", "1",
                ]
                command = [
                    str(bridge), "conda", "run", "-n", "crystal_wsl", "python", str(guard),
                    "--output-dir", str(guard_dir),
                    "--min-available-gib", "3", "--min-swap-free-gib", "4",
                    "--interval", "0.5", "--", *train_command,
                ]
