...
def sample_top_p(key, logits, p, temperature):
    ...
    if p < 1.0:
        ...
        mask =  jnp.concatenate([jnp.zeros((batchsize, 1)),  # at least keep the leading one
                                (cumulative_probs > p)[:, :-1] #这儿是去掉最后一项的，因为只要是p<1,总是不可能取最后一项的
                                ], axis=1)
    ...
    return samples #这里返回的是选中的序号而不是真实的值
...
def make_sample_crystal(transformer, n_max, atom_types, wyck_types, Kx, Kl, w_mask, top_p, temperature, K=0, g=None, atom_mask=None, spg_mask=None):
    ...
    def sample_crystal(key, params, batchsize, composition):
        ...
        def body_fn(i, state):
            ...
            # (1) W 
            w_logit = inference(transformer, params, composition, G, W, A, X, Y, Z)[1][:, 5*i] # 先在(g_logit, h)然后取对应的位置，(batchsize, output_size)
            ...
            L = L.at[:, i].set(lattice_params)  #每一轮都要保留晶格参数，因为batch里面不同的样本可能结束的位置不一样
        ...
        if g is None: 
            if K == 0: 
                ...
                g_logits_topk = jnp.argsort(-g_logit[0][offset:])[:K]  #这里g_logit每一行都一样，所以干脆直接取第一行
        ...
        key, W, A, X, Y, Z, L = jax.lax.fori_loop(0, n_max, body_fn, (key, W, A, X, Y, Z, L)) #所有的采样过程基本上在这一行
        ...
        L = jax.random.normal(key_l, (batchsize, 6)) * sigma*jnp.sqrt(temperature) + mu # (batchsize, 6) 相当于在生成的正太分布里面采样
...
def make_update_lattice(transformer, params, atom_types, Kl, top_p, temperature):
    ...
    def update_lattice(key, G, XYZ, A, W):
        ...
        h = jax.vmap(transformer, in_axes=(None, None, 0, 0, 0, 0, 0, None))(params, key, G, XYZ, A, W, M, False) #怎么回事，这儿有错误
